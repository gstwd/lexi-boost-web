...
export const reviewsApi = {
  // 复习调度管理
  async getReviewSchedules(filters?: ReviewFilters, page = 1, limit = 20): Promise<ApiResponse<PaginatedResponse<ReviewSchedule>>> {
  ...
  // 复习会话管理
  async startReviewSession(wordRecordIds: number[]): Promise<ApiResponse<{ sessionId: string, reviews: ReviewSchedule[] }>> {
  ...
  async endReviewSession(sessionId: string): Promise<ApiResponse<{
  ...
  // 复习历史
  async getReviewSessions(page = 1, limit = 20, filters?: {
    startDate?: string
    endDate?: string
    reviewType?: ReviewType
  ...
  // 复习统计
  async getReviewStats(period: 'daily' | 'weekly' | 'monthly', limit = 30): Promise<ApiResponse<{
    dates: string[]
    reviewCounts: number[]
  ...
  // 复习设置和偏好
  async getReviewSettings(): Promise<ApiResponse<{
    dailyGoal: number
  ...
  // 批量操作
  async markMultipleAsReviewed(scheduleIds: number[], result: 'correct' | 'incorrect' | 'skip'): Promise<ApiResponse<{
    updated: number
