...
export const useReviewsStore = defineStore('reviews', () => {
  ...
  const submitReviewResult = async (result: {
    ...
    try {
      ...
      if (scheduleIndex !== -1) {
        schedules.value[scheduleIndex].nextReviewDate = response.nextReviewDate
  ...
  const endReviewSession = async () => {
    ...
    try {
      ...
      // 更新统计数据
      stats.value.completedToday += response.completedReviews
  ...
  const updateSettings = async (newSettings: Partial<typeof settings.value>) => {
    ...
    try {
      await reviewsApi.updateReviewSettings(newSettings)
